...
#define WIMAX_CPU1ONLINE_PATH \
"/sys/devices/system/cpu/cpu1/online"
#define WIMAX_CPU0SCALING_PATH \
"/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor"

#define MEDIA_DISCONNECTED 0
#define MEDIA_CONNECTED 1
#define CMC_MP_SKB_POOL 4
...
struct net_adapter {
	...
	struct work_struct		wimax_reset;
...
#define hwSdioReadCounterRx(Adapter, pLen, pRIdx, Status)\
{\
    *pLen = 0;\
    *pLen = sdio_readw(Adapter->func,\
			(SDIO_RX_BANK_ADDR + (*pRIdx * SDIO_RXBANK_SIZE)) ,\
			Status);\
	if (*Status == 0)\
		*pLen |= sdio_readw(Adapter->func,\
			(SDIO_RX_BANK_ADDR + (*pRIdx * SDIO_RXBANK_SIZE) + 2),\
			Status) << 16;\
    if (*pLen > SDIO_RXBANK_SIZE) {\
	*pLen = 0;\
    } \
}
