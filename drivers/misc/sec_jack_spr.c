...
#define	WAKE_LOCK_KEY_TIME	(HZ * 2)		/* 2 sec */
#endif
...
#if defined(CONFIG_MACH_C1_KDDI_REV00)
		wake_lock_timeout(&hi->key_wake_lock, WAKE_LOCK_KEY_TIME);
#endif
...
static int sec_jack_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_MACH_C1_KDDI_REV00)
	wake_lock_init(&hi->key_wake_lock, WAKE_LOCK_SUSPEND, "sec_jack_key");
#endif
	...
	ret = request_threaded_irq(hi->det_irq, NULL,
				   ...
				   IRQF_DISABLED, "sec_headset_detect", hi);
	if (ret) {
...
#if defined(CONFIG_MACH_C1_KDDI_REV00)
	wake_lock_destroy(&hi->key_wake_lock);
#endif
