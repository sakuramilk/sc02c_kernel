...
enum {
	LCD_TYPE_DEFAULT = 0,
	LCD_TYPE_A1_SM2_REVD,	/* 12h 4eh: A1 SM2 : Manual Rev D */
	LCD_TYPE_A1_SM2_REVF,	/* 12h 8eh: A1 SM2 : Manual Rev F */
	LCD_TYPE_A2_SM2_REVH,	/* a2h aeh: A2 SM2 : Manual Rev H */
	LCD_TYPE_MAX,
};
...
#define ELVSS_MIN_VOLTAGE	62
#define ELVSS_MAX_VOLTAGE	32
...
static int s6e8ax0_ldi_init(struct lcd_info *lcd)
{
	...
	if (lcdtype == LCD_TYPE_A1_SM2_REVD)
		s6e8ax0_write(lcd,SEQ_PANEL_CONDITION_SET_REVD, ARRAY_SIZE(SEQ_PANEL_CONDITION_SET_REVD));
	else if (lcdtype == LCD_TYPE_A1_SM2_REVF)
		s6e8ax0_write(lcd,SEQ_PANEL_CONDITION_SET_REVF, ARRAY_SIZE(SEQ_PANEL_CONDITION_SET_REVF));
	else if (lcdtype == LCD_TYPE_A2_SM2_REVH)
		s6e8ax0_write(lcd,SEQ_PANEL_CONDITION_SET_REVF, ARRAY_SIZE(SEQ_PANEL_CONDITION_SET_REVF));
	else
		s6e8ax0_write(lcd,SEQ_PANEL_CONDITION_SET_REVF, ARRAY_SIZE(SEQ_PANEL_CONDITION_SET_REVF));
	...
	if (lcdtype == LCD_TYPE_A1_SM2_REVD)
		s6e8ax0_write(lcd,SEQ_GAMMA_CONDITION_SET_REVD, ARRAY_SIZE(SEQ_GAMMA_CONDITION_SET_REVD));
	else if (lcdtype == LCD_TYPE_A1_SM2_REVF)
		s6e8ax0_write(lcd,SEQ_GAMMA_CONDITION_SET_REVF, ARRAY_SIZE(SEQ_GAMMA_CONDITION_SET_REVF));
	else if (lcdtype == LCD_TYPE_A2_SM2_REVH)
		s6e8ax0_write(lcd,SEQ_GAMMA_CONDITION_SET_REVH, ARRAY_SIZE(SEQ_GAMMA_CONDITION_SET_REVH));
	else
		s6e8ax0_write(lcd,SEQ_GAMMA_CONDITION_SET_REVF, ARRAY_SIZE(SEQ_GAMMA_CONDITION_SET_REVF));
...
{
	u8 volt;
	//don't use this function - only AID value and min/max voltage printting

	volt = ELVSS_MIN_VOLTAGE;
	printk("[ELVSS_MIN_VOLTAGE]:%s ELVSS_MIN_VOLTAGE :%d\n", __func__, volt);
	volt = ELVSS_MAX_VOLTAGE;
	printk("[ELVSS_MAX_VOLTAGE]:%s ELVSS_MAX_VOLTAGE :%d\n", __func__, volt);

	if (limit == 0x4)
		printk("[NO_AID]:%s AID value  : 0x%X\n", __func__, limit);
	else if (limit == 0x5)
		printk("[USE_AID_1]:%s AID value  : 0x%X\n", __func__, limit);
	else if (limit == 0x6)
		printk("[USE_AID_2]:%s AID value  : 0x%X\n", __func__, limit);
	else if (limit == 0x7)
		printk("[USE_AID_3]:%s AID value  : 0x%X\n", __func__, limit);
	else
		printk("[ERROR:AID]:%s undefined AID value  : 0x%X\n", __func__, limit);
...
u8 get_elvss_value(struct lcd_info *lcd, u32 brightness)
{
	...
	//printk("[ELVSS reference]:%s ELVSS reference 1  : 0x%X\n", __func__, lcd->elvss.reference);

	offset = get_offset_brightness(brightness);
	ref += offset;

	//printk("[ELVSS reference]:%s ELVSS reference 2  : 0x%X\n", __func__, ref);
	...
	//printk("[ELVSS]:%s setting elvss value : 0x%X\n", __func__, ref);
...
int s6e8aa0_update_elvss(struct lcd_info *lcd, u32 brightness)
{
	...
	//printk("elvss reg : %02x ", elvss_cmd[2]);
...
int s6e8aa0_read_id(struct lcd_info *lcd)
{
	...
	lcd->elvss.limit = (rd_buf[2] & 0xe0) >> 5 ; //0x4 : AID none, 0x5 : AID_1, 0x6 : AID_2, 0x7 : AID_3
	lcd->elvss.reference = rd_buf[2] & 0x1f;
	
	printk("Dynamic ELVSS Information\n");
	printk("limit(AID)    : %02x\n", lcd->elvss.limit);
...
static int s6e8ax0_probe(struct device *dev)
{
	...
	memcpy(&SEQ_PANEL_CONDITION_SET, &SEQ_PANEL_CONDITION_SET, sizeof(SEQ_PANEL_CONDITION_SET));
	lcd->gamma_table = gamma22_table;
...
read_retry:
	...
	if ((id_buf[0] == 0x12) && (id_buf[1] == 0x4e))
		lcdtype = LCD_TYPE_A1_SM2_REVD;
	else if ((id_buf[0] == 0x12) && (id_buf[1] == 0x8e))
		lcdtype = LCD_TYPE_A1_SM2_REVF;
	else if ((id_buf[0] == 0xa2) && (id_buf[1] == 0xae))
		lcdtype = LCD_TYPE_A2_SM2_REVH;
	else
		lcdtype = LCD_TYPE_A1_SM2_REVF;

	lcd->support_elvss = 1;
	lcd->elvss.limit = (id_buf[2] & 0xe0) >> 5 ; //0x100 : AID none, 0x101 : AID_1, 0x110 : AID_2, 0x111 : AID_3
	lcd->elvss.reference = id_buf[2] & 0x1f;
