...
static void s5p_dsim_frame_done_enable(int state)
{
	u32 intmsk;

	if (dsim.mipi_ddi_pd->resume_complete == 1) {
			...
			case S5P_DSIM_INT_MSK_FRAME_DONE:
				...
				if (s3cfb_vsync_status_check() && dsim.mipi_ddi_pd->resume_complete == 1) {
					if (completion_done(&dsim_wr_comp) && completion_done(&dsim_rd_comp)){
						s5p_dsim_toggle_hs_clock(dsim.reg_base);
						//printk("S5P_DSIM_INT_MSK_FRAME_DONE-3\n");
					}						
					s5p_dsim_frame_done_enable(1);
					schedule_delayed_work(&dsim_work, msecs_to_jiffies(2500));
				}
				break;
			}
		}
...
void s5p_dsim_shutdown(struct platform_device *pdev)
{
	u32 int_stat = 0;
