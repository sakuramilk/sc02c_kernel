...
const unsigned char gamma_300cd_8e[] = {
    0x71, 0x31, 0x7B, 0xA4, 0xB6, 0x95, 0xA9, 0xBC, 0xA2,
...
const unsigned char gamma_300cd_ae[] = {
	0x5F, 0x2E, 0x67, 0xAA, 0xC6, 0xAC, 0xB0, 0xC8, 0xBB,
...
const unsigned char gamma_id_list[GAMMA_300CD_MAX] = {
	0x23, 0x33, 0x43, 0x4E, 0x53, 0x63, 0x73, 0x83, 0x8E, 0xAE
};
	...
	for (c = CI_RED; c < CI_MAX; c++) {
		for (i = IV_1; i < IV_255; i++) {
			...
			smart->adjust_mtp[c][calc_seq[i]] = t2;
                        if(t2>255){
                         printk("V Level cal_seq t2-255 : %d \n", t2-255);
                         t2=255;
                        }
...
#if 1
	for (i = 0; i < 24; i++)
		printk(" Default Gamma Index : %d : %x\n", i, smart->default_gamma[i]);
#endif
	return 0;
}
