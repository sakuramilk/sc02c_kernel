...
static void dpram_phone_reset(void)
{
	...
	if(g_dpram_wpend == IDPRAM_WPEND_LOCK)
		g_dpram_wpend = IDPRAM_WPEND_UNLOCK; /* ram write unlock */
...
static int get_minor_start_index(int id)
{
	...
	case 7:		start = 2;    break;
	case 9:		start = 3;    break;
	case 27:	start = 4;    break;
	case 29:	start = 5;    break;
	default:	start = 0;
    }
...
//__raw_writel(__raw_readl(S5PV210_EINT1FLTCON0)|(FILTER_EINT9_EN & (~FILTER_EINT9_SEL_DEGIT)),S5PV210_EINT1FLTCON0);
	...
	#if defined(CONFIG_MACH_C1_KDDI_REV00)
	{
		...
		if (sec_isLpmMode() == 0) // NORMAL MODE (Not lpm mode)
		{
			printk(KERN_ERR "Normal mode, not LPM so make the modem on\n");
			...
			printk(KERN_ERR "LPM MODE.  Dont start modem & PDP.\n");
	}
	#else
...
static int __devinit dpram_probe(struct platform_device *dev)
{
    ...
    #if defined(CONFIG_MACH_C1_KDDI_REV00)
    {
        ...
        if (sec_isLpmMode() == 0) // NORMAL MODE (Not lpm mode)
        {
	    printk(KERN_ERR "Normal mode, not LPM so do multipdp_init\n");
	    multipdp_init();
         }
	else
		printk(KERN_ERR "LPM MODE.	Dont do multipdp_init.\n");
    }
    #else
    multipdp_init();
    #endif
...
static int __init dpram_init(void)
{
	...
	if (sec_isLpmMode())
	{
		printk(KERN_ERR "LPM MODE (dpram_init)!\n");
		return 0;
	}
	#endif	
