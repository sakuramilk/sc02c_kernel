...
static bool is_ovp_status=false;
#endif
...
static void smb328_test_read(struct i2c_client *client)
{
	...
	for(addr=0;addr<0x0c;addr++)
	{
		...
		dev_info(&client->dev, "smb328 addr : 0x%02x data : 0x%02x\n", addr,data);
	}
...
static int smb328_get_charging_status(struct i2c_client *client)
{
	...
	smb328_i2c_read(chg->client, SMB328A_BATTERY_CHARGING_STATUS_A, &data_a);
	dev_info(&client->dev, "%s : charging status A(0x%02x)\n",
		__func__, data_a);
	smb328_i2c_read(chg->client, SMB328A_BATTERY_CHARGING_STATUS_B, &data_b);
	dev_info(&client->dev, "%s : charging status B(0x%02x)\n",
		__func__, data_b);
	smb328_i2c_read(chg->client, SMB328A_BATTERY_CHARGING_STATUS_C, &data_c);
...
#if 1	/* write 0xAA in register 0x30 to reset watchdog timer, it can replace this work-around */
	if (chg->is_enable && chg->cable_type == CABLE_TYPE_USB) {
		if ((data_c & 0x30) == 0x20) {	/* safety timer activated */
			/* reset charger */
			dev_info(&client->dev, "%s : Reset charger, safety timer is activated!\n",
				__func__);
	...
	/* At least one charge cycle terminated, Charge current < Termination Current */
	if ((data_c & 0xc0) == 0xc0) {
		status = POWER_SUPPLY_STATUS_FULL;	/* top-off by full charging */
		goto charging_status_end;
	}

	/* Is enabled ? */
	if (data_c & 0x01) {
		/* check for 0x30 : 'safety timer' (0b01 or 0b10) or 'waiting to begin charging' (0b11) */
		/* check for 0x06 : no charging (0b00) */
		if ((data_c & 0x30) || !(data_c & 0x06))  { /* not charging */
...
static int smb328_get_charging_health(struct i2c_client *client)
{
...
#if defined (CONFIG_MACH_Q1_CHN)
	{
		...
		smb328_i2c_read(chg->client, 0x37, &data_chn);
			dev_info(&client->dev, "%s : charging interrupt status C(0x%02x)\n",
				__func__, data_c);
...
static void smb328a_charger_function_conrol(struct i2c_client *client)
{
	...
	if (chip->is_otg) {
		dev_info(&client->dev, "%s : OTG is activated. Ignore command! (type:%d, enable:%s)\n",
			__func__, chip->cable_type, chip->is_enable?"true":"false");
		return;
	}
	...
	if(!chip->is_enable) {
		...
		reg = SMB328A_CLEAR_IRQ;	/* reset watchdog timer if it occured */
		val = smb328_i2c_read(client, reg, &data);
		if (val >= 0) {
			data = (u8)val;
			dev_info(&client->dev, "%s : reg (0x%x) = 0x%x\n",
		...
		if (val >= 0) {
			...
			if (chip->cable_type == CABLE_TYPE_AC) {
				set_data = 0xb7; /* fast 1000mA, termination 200mA */
			} else if (chip->cable_type == CABLE_TYPE_MISC) {
				set_data = 0x57; /* fast 700mA, termination 200mA */
			} else
				set_data = 0x17; /* fast 500mA, termination 200mA */
		...
		if (val >= 0) {
			...
			dev_info(&client->dev, "%s : reg (0x%x) = 0x%x\n",
				__func__, reg, data);
			if (chip->cable_type == CABLE_TYPE_AC) {
				set_data = 0xb0; /* input 1A, threshold 4.25V, AICL enable */
			} else if (chip->cable_type == CABLE_TYPE_MISC) {
				set_data = 0x50; /* input 700mA, threshold 4.25V, AICL enable */
			} else
				set_data = 0x14; /* input 450mA, threshold 4.25V, AICL disable */
#if defined (CONFIG_MACH_Q1_CHN)
				set_data &= ~(0x10); /* turn off pre-bias for ovp */
#endif
			if (data != set_data) {
				data = set_data;
				if (smb328_i2c_write(client, reg, data) < 0)
		...
		if (val >= 0) {
			...
			dev_info(&client->dev, "%s : reg (0x%x) = 0x%x\n",
				__func__, reg, data);
			if (data != 0x4e) {	// 0x4c -> 0x4e (watchdog timer enabled - SUMMIT)
				data = 0x4e;	// 0x4c -> 0x4e (watchdog timer enabled - SUMMIT)
				if (smb328_i2c_write(client, reg, data) < 0)
					pr_err("%s : error!\n", __func__);
				val = smb328_i2c_read(client, reg, &data);
				if (val >= 0) {
		...
		if (val >= 0) {
			...
			dev_info(&client->dev, "%s : reg (0x%x) = 0x%x\n",
				__func__, reg, data);
			if (chip->cable_type == CABLE_TYPE_AC) {		/* turn on charger */
				data = 0x8c;
			} else if (chip->cable_type == CABLE_TYPE_MISC) {
				data = 0x88;
			} else
			    ...
			    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (chip->is_enable) {
		smb328_i2c_read(chip->client, SMB328A_INPUT_AND_CHARGE_CURRENTS, &data);
		switch (data >> 5) {
		case 0:
			val->intval = 450;
			break;
			    ...
			    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGE_TYPE:	/* only for OTG support */
...
static irqreturn_t smb328_irq_thread(int irq, void *data)
{
	...
	if (data1 & 0x04) { //if usbin(usb vbus) is in over-voltage status.		
	...
	if (ret) {
		dev_err(&chip->client->dev, "%s: error from topoff_cb(%d)\n",
			__func__, ret);
		return IRQ_HANDLED;
	}
#endif
...
static int smb328_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#if defined (CONFIG_MACH_Q1_CHN)
#if defined (CONFIG_MACH_Q1_CHN) && defined (CONFIG_SMB328_CHARGER)
		s3c_gpio_cfgpin(chip->pdata->gpio_chg_ing, S3C_GPIO_SFN(0xf)); //set external interrupt
#endif
