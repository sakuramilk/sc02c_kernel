...
#if USING_MALI_PMM_EARLYSUSPEND == 0
#undef CONFIG_HAS_EARLYSUSPEND
#endif /* USING_MALI_PMM_EARLYSUSPEND */
...
{
	int err = 0;
	MALI_PRINT(("OSPMM: mali_pm_os_suspen\n"));
...
static int mali_pwr_suspend_notifier(struct notifier_block *nb,unsigned long event,void* dummy)
{
	...
	switch (event)
	{
		case PM_SUSPEND_PREPARE:
			MALI_PRINT(("OSPMM: mali_pwr_suspend_notifier : PM_SUSPEND_PREPARE\n"));
			err = mali_pm_suspend(NULL);
		break;

		case PM_POST_SUSPEND:
			MALI_PRINT(("OSPMM: mali_pwr_suspend_notifier : PM_POST_PREPARE\n"));
...
{
	int err = 0;
	MALI_PRINT(("OSPMM: mali_pm_os_resume\n"));
	err = mali_pm_resume(NULL);
	return err;
}
#endif /*CONFIG_HAS_EARLYSUSPEND*/
...
static void mali_pm_early_suspend(struct early_suspend *mali_dev)
{
	switch(mali_dev->level)
	{
		...
		case EARLY_SUSPEND_LEVEL_DISABLE_FB:
			...
			MALI_DEBUG_PRINT(1, (">MALI ES\n"));
#if MALI_DVFS_ENABLED
			if( mali_dvfs_is_running()!=0 )
			{
				MALI_DEBUG_PRINT(1, ("error: suspend during dvfs work\n"));
			}
#endif
			_mali_osk_lock_wait(lock, _MALI_OSK_LOCKMODE_RW);
#if MALI_DVFS_ENABLED
