...
#if defined(CONFIG_MACH_C1_KDDI_REV00)
	val = 0; //ssong100805. HW team requests for me to make RF UART calibration work fine. 
#endif
...
REGULATOR_INIT(ldo13, "VCC_3.0V_LCD", 3100000, 3100000, 0,
		REGULATOR_CHANGE_STATUS, 1);
#else
...
static struct max8997_regulator_data max8997_regulators[] = {
	...
	|| defined(CONFIG_MACH_TALBOT_REV02) || defined(CONFIG_TARGET_LOCALE_NA)
...
#if defined(CONFIG_MACH_Q1_REV02) || defined(CONFIG_PANEL_EPIC2)
...
static struct stmpe_kpad_platform_data stmpe_platform_info = {
	.rows = STMPE1601_KPAD_MAX_ROWS,
	.cols = STMPE1601_KPAD_MAX_COLS,
	.gpio_irq = GPIO_KEY_INT,
...
/* I2C1 */
#if defined(CONFIG_OPTICAL_GP2A) || defined(CONFIG_OPTICAL_GP2AP020)
...
static struct i2c_board_info i2c_devs2[] __initdata = {
#ifdef CONFIG_MACH_C1_NA_SPR_EPIC2_REV00
	{
		...
		.platform_data = &stmpe_platform_info,
#endif
	},
#endif
};
...
#if defined(CONFIG_MACH_C1_KDDI_REV00)
		.irq = IRQ_EINT(27), 	//ssong110908. NFC pin setting. XEINT_27, GPX3[3]
#else
		.irq = IRQ_EINT(15),
#endif
...
#if !defined(CONFIG_PANEL_C1_NA_SPR)
...
static int reset_lcd(void)
{
	...
	gpio_direction_output(S5PV310_GPY4(5), 0);
...
#if defined(CONFIG_MACH_C1_KDDI_REV00)
static bool IsRegDisInLPM = false;
int sec_disable_regulator(const char *reg_nm )
{
	struct regulator *regulator =  NULL;

	if (reg_nm)
		printk("%s:  Disabling Regulator -> %s\n", __func__, reg_nm);

	regulator = regulator_get(NULL, reg_nm);
	if (IS_ERR(regulator))
      		  return 0;

	if (regulator_is_enabled(regulator))  
	{
		printk("%s:  Forced Disable -> %s\n", __func__, reg_nm);
       		regulator_force_disable(regulator);
	}
	regulator_put(regulator);
...
int sec_isLpmMode(void)
{
	u32 val = __raw_readl(S5P_INFORM2);

	pr_err("%s: LP charging:%d\n", __func__, val);

	if (val && IsRegDisInLPM == false)
	{
		sec_disable_regulator("vdd_g3d");
		sec_disable_regulator("vdd_arm");
		sec_disable_regulator("vdd_int");
		sec_disable_regulator("vhsic");
...
static int max8903_cfg_gpio(void)
{
	...
	s3c_gpio_cfgpin(GPIO_CHG_USUS, S3C_GPIO_OUTPUT);
	s3c_gpio_setpull(GPIO_CHG_USUS, S3C_GPIO_PULL_NONE);
	gpio_set_value(GPIO_CHG_USUS, GPIO_LEVEL_LOW);
...
#if defined(CONFIG_VIDEO_S5K6AAFX) || defined(CONFIG_VIDEO_S5K5BAFX) || defined(CONFIG_VIDEO_S5K5BBGX) || defined(CONFIG_VIDEO_SR130PC10) || defined(CONFIG_VIDEO_SR200PC20)
	&s3c_device_i2c12,
#endif
#ifdef CONFIG_SND_SOC_MIC_A1026
	&s3c_device_i2c13,
